...
public:
	...
	explicit uifstream(const std::string &filename, std::ios_base::openmode mode = std::ios_base::in)
			: std::wifstream(filename, mode) {
		locale_init();
	}
...
public:
	uofstream() : std::wofstream() {
		locale_init();
	}

	explicit uofstream(const char *filename, std::ios_base::openmode mode = std::ios_base::in)
			: std::wofstream(filename, mode) {
		locale_init();
	}
	explicit uofstream(const std::string &filename, std::ios_base::openmode mode = std::ios_base::in)
			: std::wofstream(filename, mode) {
		locale_init();
	}
...
inline String ToLower(const String & sWord) {
	return ToLower(String(sWord));
}
...
inline std::vector<String> ParseWords(const String & sField) {
	...
	while (iCurrentPos < sField.length()) {
		if (sField[iCurrentPos] == L' ') {
			auto sCurrentWord = sField.substr(iBeginPos, iCurrentPos - iBeginPos);
			...
			for (uint32_t i = 0; i < regexMatch.size(); ++i) {
				...
				//auto word = ToLower(ToWstring(stemmer.Process(ToString(regexMatch.str(i)))));
			}
		}
		iCurrentPos++;
	}
