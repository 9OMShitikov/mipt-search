...
Socket::Socket(const std::string &address) : Poco::Net::Socket(new ServerSocketImpl()) {
...
void NetworkServer::defineOptions(Poco::Util::OptionSet & tOptions) {
	tOptions.addOption(
            Poco::Util::Option("config", "C")
                    ...
                    .repeatable(false)
                    .argument("path")
                    .callback(Poco::Util::OptionCallback<NetworkServer>(this, &NetworkServer::handleConfig)));
}

void NetworkServer::handleConfig(const std::string &name, const std::string &value) {
...
int NetworkServer::main(const std::vector<std::string> & args) {
	...
	Poco::Net::HTTPServerParams::Ptr pParameters = new Poco::Net::HTTPServerParams();
    ...
    Poco::Net::HTTPServer tServer(new HandlerFactory(tStorage), tSocket, pParameters);
